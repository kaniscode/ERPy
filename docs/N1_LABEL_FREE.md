...
[Figure S1](../validation/manuscript_figures/README.md) displays every injection-calibration cell. The saved pure-noise family interval remains in the retained summary and tables. Its contact-level heatmaps are descriptive because the two targets within each family share noise; they are not ten independent Bernoulli trials per cell. The plotting recipe reads retained counts and does not rerun or alter inference.
...
The [retained evidence](../validation/n1_label_free/README.md) contains the exact
acknowledged protocols, all family-level calls, human-reference scoring,
decision-stage counts, the sole ablation and secondary comparisons, plus the
synthetic calibration. Label-blind prediction and human-label scoring are
separate scripts. All 577 original families and 34,926 finite early p values
are retained; 50 finite contacts have no available human rater and 639 are
ineligible for later scoring, but remain in BH.
...
python validation/predict_n1_label_free.py \
  ...
  --output /path/to/new-predictions
python validation/score_n1_label_free.py \
  --protocol validation/n1_label_free/protocols/LABEL_FREE_N1_PROTOCOL.json \
  --predictions /path/to/new-predictions --output /path/to/new-scoring
python validation/summarize_n1_nominal_stage.py \
  --scoring /path/to/new-scoring --output /path/to/new-stage-diagnostics
```

These commands use complete, checksum-verified public derived data without a
raw download or model fitting. A new raw recording uses the array API above.
The nine existing executed notebooks remain unchanged; notebook 08 demonstrates
the secondary supervised model and is not a label-free example.
