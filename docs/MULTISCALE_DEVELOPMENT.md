...
The saved tables retain both development and confirmation outcomes and the
fixed analysis plans. The optional API above is available for separately
declared experiments; the single-window default remains unchanged. The saved
runs used Python 3.12.5, NumPy 1.26.4, SciPy 1.13.1 and pandas 2.2.2, with one
BLAS thread per worker. Historical source hashes describe those executions;
the current library implementation is the distributed source.
