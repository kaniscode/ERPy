...
## Rebuilding the published outputs
...
starting Jupyter or the execution script. The loader checks the input checksum,
...
With the authorized cohort input configured, reproduce all eight notebooks
with a fresh kernel for each notebook:

```text
python -m pip install ".[viz,validation]" nbformat nbclient ipykernel kaleido
...
`scripts/build_example_notebooks.py` regenerates the source-only notebooks;
run `scripts/execute_example_notebooks.py` afterward to restore published
outputs. The release audit permits outputs only in the eight curated examples
with complete execution provenance. Raw recordings, validation workspaces,
unreviewed notebook outputs, and local machine paths remain excluded.
