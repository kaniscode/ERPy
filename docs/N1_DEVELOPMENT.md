...
## Reproduce from immutable derived inputs

Use Python 3.11+ and run from the repository root. Use a new output directory each time; scripts
refuse to overwrite an existing development run. No network or raw recordings
are needed to reproduce fitting from the packaged feature table and matched
saved inference/annotation table.

```bash
python -m pip install -e ".[n1-training]"
python validation/verify_n1_artifacts.py
python validation/optimize_n1.py \
  --records validation/frozen_results/erdetect/record_predictions_and_labels.csv.gz \
  --features validation/frozen_results/n1/features \
  --output validation/work/n1_nested_replay
python validation/summarize_n1_development.py \
  --records validation/frozen_results/erdetect/record_predictions_and_labels.csv.gz \
  --features validation/frozen_results/n1/features \
  --run validation/work/n1_nested_replay \
  --output validation/work/n1_secondary_replay
```

The saved run used Python 3.12.5, NumPy 1.26.4, SciPy 1.13.1, pandas 2.2.2
and scikit-learn 1.4.2. Other versions within the scripts' Python 3.11+ requirement
may change numerical rounding; the expanded version matrix has not been verified.
Timestamps and resulting provenance hashes differ in a fresh run, so replay
comparisons should join predictions by canonical identity and compare scores,
decisions and summary values rather than requiring every JSON byte to match.

To regenerate the features, first reconstruct the public epoch caches using
the [external-validation instructions](EXTERNAL_VALIDATION.md), then run:

```bash
python validation/extract_n1_features.py \
  validation/work/erdetect/epochs validation/work/n1_features_replay --workers 4
```

Substitute the actual reconstructed epoch folder if different. The feature
manifest records all 596 epoch hashes; matching the source hashes is necessary
for a like-for-like replay. Extraction does not read annotations or archived
calls. Authentic source/protocol snapshots, including the earlier extraction
protocol revision, are retained with the evidence. Do not normalize their
line endings or edit pinned data in place. New inputs/settings belong in a
new manifest and output directory.
