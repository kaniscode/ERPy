...
Figure6 compares saved ERPy15–300 ms detector calls with released human negative-N1 ratings in public OpenNeuro ds004774. Its analysis and examples remain unchanged. Figure7 separately compares the [fixed label-free N1 detector](N1_LABEL_FREE.md) with archived ER-detect; the trained morphology and hybrid models remain secondary comparisons. Historical calls, models and prediction records are retained.
...
From a checkout with the scientific development dependencies installed:

```bash
python validation/analyze_annotation_agreement.py --verify
python validation/analyze_annotation_agreement.py --output /path/to/new-agreement-results
```

The first command verifies hashes and recomputes every retained aggregate table in a temporary directory. The second writes a new copy. Both use saved public predictions, labels and the pinned four-example trace summaries; no raw download is needed. To reconstruct the trace summaries from the original recovered public epoch caches, also pass `--epochs /path/to/ds004774/epochs`. Each NPZ must match its saved prediction hash. The full contact matrix and individual-rater records remain in `validation/frozen_results/erdetect`.

`validation/annotation_agreement/manifest.json` binds the inputs and outputs. `source_traces` holds only the four selected public-data summaries and reconstruction metadata, with dataset-relative paths. Current publication artwork, captions and reproduction recipes are indexed in [the figure README](../validation/manuscript_figures/README.md).

Figure7's paired intervals are nominal conditional intervals for the fixed label-free calls and archived comparator on the same participants. An interval including zero does not demonstrate equivalence or noninferiority. The cohort had already been examined before the fixed rule was proposed, and no threshold was selected on its new scores. The previously retained six classifier intervals in `annotation_agreement/figure7_paired_intervals.csv` remain valid secondary supervised evidence; they are not the current primary Figure7 data.
