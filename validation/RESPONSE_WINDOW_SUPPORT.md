...
The current detector rejects an epoch that does not cover the effective declared response window within half a sample (plus a 1e-12-second floating-point tolerance). Such an input receives `insufficient_data`, unavailable p-values and zero randomizations. The result retains the requested window. A deliberately declared shorter window remains usable; the baseline still uses the latest available sample-matched segment within its declared candidate interval.
