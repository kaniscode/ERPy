...
Read the CSV tables directly; the manifest records the fixed input and output identities. Historical source hashes describe the executed analysis.
...
- `supervised_paired_intervals.csv`: six retained secondary supervised N1 comparison intervals, without new fits or resampling. These historical classifier comparisons are not the primary label-free comparison data; see `../n1_label_free/empirical_scoring/` and the separately reported `../n1_label_free/nominal_stage/`.
- `source_traces`: pinned summaries and metadata needed to reproduce the example summaries without redistributing full source recordings.
