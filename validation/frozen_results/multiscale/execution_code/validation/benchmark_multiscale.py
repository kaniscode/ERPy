...
METHODS = ("early", "broad", "late", "matched_broad", "multiscale")
...
def summarize(families, output):
    ...
    for n in ["all",8,12,24]:
        ...
        for comparison in ["early","broad","matched_broad"]:
            diff=wide["multiscale"]-wide[comparison];samples=np.zeros(2000)
            ...
            paired.append(dict(n_trials=n,comparison=comparison,families=len(wide),multiscale_recovery=wide.multiscale.mean(),
                reference_recovery=wide[comparison].mean(),paired_difference=diff.mean(),ci_low=lo,ci_high=hi,
                multiscale_only=int(((wide.multiscale==1)&(wide[comparison]==0)).sum()),reference_only=int(((wide.multiscale==0)&(wide[comparison]==1)).sum())))
...
def run(output, workers, real=False):
    ...
    manifest={'created_utc':datetime.now(timezone.utc).isoformat(),'analysis_plan_sha256':sha(plan),'predeclared_families':len(specs),'completed_families':len(specs)-len(failures),
        'failed_families':len(failures),'methods':METHODS,'elapsed_wall_seconds':elapsed,'worker_processes':workers,'python':platform.python_version(),'numpy':np.__version__,
    ...
    manifest['outputs']={p.name:sha(p) for p in output.iterdir() if p.is_file() and p.name not in ['run_manifest.json','analysis_plan.json','analysis_plan.sha256','analysis_plan.md','analysis_plan_initial.json']}
    (output/'run_manifest.json').write_text(json.dumps(manifest,indent=2))
    print(json.dumps(manifest,indent=2),flush=True)


if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--output-dir',type=Path,required=True);ap.add_argument('--workers',type=int,default=3);ap.add_argument('--real-baseline',action='store_true');args=ap.parse_args()
    run(args.output_dir,args.workers,args.real_baseline)
