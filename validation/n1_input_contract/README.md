...
The hybrid predictor requires the trained early-window inference configuration and matching source/trial provenance. `prepare_n1_hybrid_inputs` computes both paths from the same arrays and retains their separate finite masks. The notebook example adapter `FrozenN1Inputs` checks the immutable public feature and inference tables, their original configuration, exact record/epoch identities and recorded trial counts. Historical clean-trial indices were not archived and remain explicitly unavailable; no indices are invented.
