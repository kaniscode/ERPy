...
## Detector–annotation agreement

The [agreement guide](docs/ANNOTATION_AGREEMENT.md) explains the current broad-response and focused N1 comparisons. The broad detector is compared with released negative-N1 ratings using equal weight per contact/pair record and participant-level uncertainty. Its four recorded examples retain their original q values and source hashes. N1-negative ratings are not a complete reference for every evoked response.

The retained aggregate tables include counts, paired differences and participant-level uncertainty. Their manifest identifies the fixed inputs and outputs.
